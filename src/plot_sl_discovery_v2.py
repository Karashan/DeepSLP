"""Plot a simplified SL Discovery prioritization curve (v2).

Reads the pre-computed ``sl_discovery_curves.tsv`` and produces a PDF figure
with the following modifications compared to the original:

1. Only the "SL count" oracle is shown (no "greedy coverage" oracle).
2. The "mean pred_prob" and "top-100 pred_prob" guided lines are removed.
3. Output is PDF at 300 dpi.

Usage
-----
    python -m src.plot_sl_discovery_v2 \
        --input outputs/external_val/external_val_old_models/query_prioritization/sl_discovery_curves.tsv \
        --output outputs/external_val/external_val_old_models/query_prioritization/sl_discovery_v2.pdf
"""
...
def parse_args() -> argparse.Namespace:
    p = argparse.ArgumentParser(description="Plot simplified SL discovery curve (v2)")
    p.add_argument(
        "--input",
        default="outputs/external_val/external_val_old_models/query_prioritization/sl_discovery_curves.tsv",
        help="Path to sl_discovery_curves.tsv",
    )
    p.add_argument(
        "--output",
        default="outputs/external_val/external_val_old_models/query_prioritization/sl_discovery_v2.pdf",
...
def main() -> None:
    ...
    # Oracle to keep
    oracle = {
        "oracle_sl": {"color": "black", "ls": ":", "lw": 1.8, "label": "Oracle (SL count)"},
    }
